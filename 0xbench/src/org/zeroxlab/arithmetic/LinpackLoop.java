...
public class LinpackLoop implements Serializable {

	private static final long serialVersionUID = 1L;
	public int numIterations;
	
//    static Bundle mInfo; 
	public static List<HashMap<String, Double>> mInfoAll  = new ArrayList<HashMap<String, Double>>(10);
	public static HashMap<String, Double> mInfo;
	
	
	
	public static String main(int i, int numIterations) {
        LinpackLoop l = new LinpackLoop();
        l.numIterations = numIterations;
        mInfo = new HashMap<String, Double>(); 
        mInfoAll.add(0, mInfo);
        return l.run_benchmark();
	}
	
    public static String main(Bundle info, int numIterations) {
        LinpackLoop l = new LinpackLoop();
        l.numIterations = numIterations;
        return l.run_benchmark();
    }
    ...
    public String run_benchmark () {
		...
		batteryThread.start();
        time = second();
    	Result result = compute_benchmark();
    	alive.set(false);
    	total = (second() - time) / numIterations;
        ...
        mInfo.put(TesterArithmetic.MFLOPS, mflops_result); 
        mInfo.put(TesterArithmetic.RESIDN, residn_result);
        mInfo.put(TesterArithmetic.TIME, time_result);
        mInfo.put(TesterArithmetic.EPS, eps_result);
        mInfo.put("JOULES", joules);
    ...
    @RemotableMethod
    public Result compute_benchmark() {
...
//          for (i = 0; i < 1000; i++) {
          ...
          return new Result(ops, norma, normx, resid, n);
    }

    private static class Result implements Serializable {
    	public double ops, norma, normx, resid;
    	public int n;
    	
    	public Result(double ops, double norma, double normx,
				double resid, int n) {
			super();
			this.ops = ops;
			this.norma = norma;
