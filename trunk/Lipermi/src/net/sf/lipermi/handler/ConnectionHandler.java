...
public class ConnectionHandler implements Runnable {
	static Logger log = Logger.getLogger(ConnectionHandler.class.getName());
    ...
    public static ConnectionHandler createConnectionHandler(Socket socket, CallHandler callHandler, IProtocolFilter filter) {
        ...
        Thread connectionHandlerThread = new Thread(connectionHandler, "proxythreadname");
                ...
                if (remoteMessage instanceof RemoteCall) {
                    ...
                    Thread delegator = new Thread(new Runnable() {
                        public void run() {
                            ...
                            try {
//                                log.debug("remoteCall: " + remoteCall.getCallId() + " - " + remoteCall.getRemoteInstance().getInstanceId());
//                                log.debug("(" + remoteCall.getCallId() + ") " + remoteCall.getArgs()[0]);
                                remoteReturn = callHandler.delegateCall(remoteCall);
//                               log.debug    ("(" + remoteCall.getCallId() + ") " + remoteReturn.getRet());
            ...
            try {
            	if (e instanceof EOFException || e instanceof SocketException) {
            		log.info("Connection closed by " + e.getClass() + ((e.getMessage() != null) ? ": " + e.getMessage() : ""));
            	}
            	else {
            			e.printStackTrace();
            	}
    ...
    private void sendMessage(IRemoteMessage remoteMessage) throws IOException {
        ...
        log.trace("prepareWrite total time: " + (((new Date()).getTime() - now.getTime())) + "ms");
        
        synchronized (this) {
        	while (remoteMessage instanceof RemoteCall && !remoteMessage.getCallId().equals(currentId.get())) {
        		try {
					wait();
	        ...
	        log.trace("sendMessage total time: " + (((new Date()).getTime() - now.getTime())) + "ms");
        }
    }
